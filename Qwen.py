...
OUTPUT_JSON_DIR = "./output_json"  # JSON 输出文件夹
...
def main():
    ...
    for filename in files:
        ...
        if main_name not in results_by_item:
            results_by_item[main_name] = {
                "item_name": main_name,
                "catalog_types": all_catalog_types,
                "additional_codes": all_additional_codes,
    ...
    # 6. 导出 JSON 报告（按件名单独输出）
    if results_by_item:
        for name, data in results_by_item.items():
            safe_name = "".join([c for c in name if c not in r'\/:*?"<>|'])
            json_path = os.path.join(OUTPUT_JSON_DIR, f"{safe_name}.json")
